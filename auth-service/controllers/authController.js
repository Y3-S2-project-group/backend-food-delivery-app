...
export const registerRestaurantManager = async (req, res) => {
    ...
    try {
        ...
        if (existing) return res.status(400).json({ msg: 'User already exists' });
        const hashed = await bcrypt.hash(password, 10);
        const user = new User({ name, email, password: hashed, role: 'restaurant' });
        await user.save();
        res.status(201).json({ msg: 'Restaurant manager registered successfully' });
    }
    catch (err) {
        if (err.name === 'ValidationError') {
            return res.status(400).json({ msg: err.message });
...
export const registerDeliveryPerson = async (req, res) => {
    ...
    try {
        ...
        if (existing) return res.status(400).json({ msg: 'User already exists' });
        const hashed = await bcrypt.hash(password, 10);
        const user = new User({ name, email, password: hashed, role: 'delivery' });
        await user.save();
